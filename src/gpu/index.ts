...
import { createComputeTexture, createSampler, createBindGroups } from './resources';
import { createShaderModules, createComputePipeline, createRenderPipeline } from './pipelines';
import { createRenderLoop } from './renderer';
...
export const initializeWebGPU = async (canvas: HTMLCanvasElement): Promise<WebGPUState> => {
    ...
    const { vertexShaderModule, fragmentShaderModule, computeShaderModule } = createShaderModules(device);
    ...
    // Create bind groups
    const { computeBindGroupA, computeBindGroupB, renderBindGroup } = createBindGroups(
        device, computePipeline, renderPipeline, textureA, textureB, sampler
    );
...
export const resizeCanvas = async (state: WebGPUState, width: number, height: number): Promise<WebGPUState> => {
    ...
    if (width !== state.textureWidth || height !== state.textureHeight) {
        ...
        const { computeBindGroupA, computeBindGroupB, renderBindGroup } = createBindGroups(
            state.device, state.computePipeline, state.renderPipeline, textureA, textureB, state.sampler
        );
